 ...
 * @date 2016-9-30
 */
public class MatchGateServerHandler extends SimpleChannelInboundHandler<DatagramPacket> {

    @Override
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        //注意，UDP的通道至始至终只有一个，关了就不能接收了。
        System.out.println("UDP通道已经连接");
        MatchGateServer.ctx = ctx;
    }
    ...
    protected void channelRead0(ChannelHandlerContext ctx, DatagramPacket
        ...
        // 消息处理。。。。。
        //DataInfo.RequestUser message = DataInfo.RequestUser.parseFrom(packet.content().nioBuffer());

        //消息发送。。。。
        DatagramPacket dp = new DatagramPacket(Unpooled.copiedBuffer("消息".getBytes()), packet.sender());
        MatchGateServer.channel.writeAndFlush(dp);
